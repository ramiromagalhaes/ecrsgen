...
int main(int argc, char * args[])
{
     ...
     */
    std::vector<float> weights(2); // will vary depending on the amount of rects
    weights[0] = 1;
    weights[1] = -1;
    ...
    for(int w = 1; w <= SAMPLE_SIZE / MIN_RECT_SIZE; w++) //width of both rectangles.
    {
        for(int h = 1; h <= SAMPLE_SIZE / MIN_RECT_SIZE; h++) //height of both rectangles
        {
            ...
            for(x[0] = 0; x[0] < SAMPLE_SIZE - MIN_RECT_SIZE; x[0]++) //for each x...
            {
                for(y[0] = 0; y[0] < SAMPLE_SIZE - MIN_RECT_SIZE; y[0]++) //...and y of the first rectangle...
                {
                    for(int k = 1; k < MAX_DIMENSIONS; k++) //...create Haar wavelets of 1 + k dimensions...
                    {
                        ...
                        {
                            for(dx[d] = 0; dx[d] <= SAMPLE_SIZE/MIN_RECT_SIZE; dx[d]++) //dx displacement of rectangle d + 1 in regard to d
                            {
                                for(dy[d] = 0; dy[d] <= SAMPLE_SIZE/MIN_RECT_SIZE; dy[d]++)  //dy displacement of rectangle d + 1 in regard to d
                                {
                                    bool overlap = false;
